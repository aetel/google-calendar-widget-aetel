<?php
/*
Plugin Name: Google Calendar Widget AETEL
Plugin URI: http://aetel.io
Description: This plugin adds a sidebar widget containing an agenda from a Google Calendar.  It is based on the Google Calendar samples and inspired by wpng-calendar.  It is smaller and simpler than wpng-calendar and allows for multiple widgets to each show their own agenda. Adjusted for AETEL Theme V2
Version: 10.0
Author: Kaz Okuda & rs
Author URI: http://ricardosa.nz
*/
...
function ko_calendar_load()
{
	class WP_Widget_KO_Calendar extends WP_Widget
	{
		...
		function widget($args, $instance)
		{
			...
			$contactEmail = empty($instance['contactEmail']) ? 'contact@email.com' : $instance['contactEmail'];
			...
			echo $before_title . '<div class="ko-calendar-widget-title" id="' . $title_id . '">'  . '</div>' . $after_title;
			...
			echo "<div class='ko-calendar-widget-loading'>Seems like we can't connect to Google Calendar, but atleast you can enjoy Pablo.</div>";
			echo '</div>';
			echo $after_widget;
			...
			<script type="text/javascript" defer="defer">
				ko_calendar.loadCalendarDefered('<?php echo $apikey ?>', '<?php echo $title_id ?>', '<?php echo $event_id ?>', <?php echo $maxresults ?>, <?php echo empty($autoexpand) ? 'false' : 'true' ?>, '<?php echo $url ?>', '<?php echo $url2 ?>', '<?php echo $url3 ?>', '<?php echo $titleformat ?>', '<?php echo $contactEmail ?>');
			</script>
			<?php
		}
		...
		function update($new_instance, $old_instance)
		{
			...
			$instance['contactEmail'] = trim(strip_tags($new_instance['contactEmail']));
		...
		{
			$defaults = array( 'title' => '', 'url' => '', 'url2' => '', 'url3' => '', 'contactEmail' => '', 'maxresults' => 5, 'autoexpand' => FALSE, 'titleformat' => '[STARTTIME - ][TITLE]');
			...
			?>
				...
				<table width="100%"><tr><td>
					...
					<label for="<?php echo $this->get_field_id('contactEmail'); ?>" style="line-height:35px;display:block;">
						<?php _e('Contact&nbsp;Email');?>:
					</label></td><td width="100%" style="width:100%">
					<input type="text" style="width:100%" id="<?php echo $this->get_field_id('contactEmail'); ?>" name="<?php echo $this->get_field_name('contactEmail'); ?>" value="<?php echo $contactEmail; ?>" />
				</td></tr></table><table width="100%"><tr><td>
