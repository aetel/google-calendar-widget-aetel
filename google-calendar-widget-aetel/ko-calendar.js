...
var ko_calendar = function ()
{
	...
	function buildDate(entry)
	{
		...
		*/
		...
		dateDisplay.innerHTML = "<i class='material-icons'>access_time</i>"+" "+dateString;
	...
	function buildLocation(entry)
	{
		...
		{
			locationDiv.innerHTML =	"<i class='material-icons'>location_on</i>";
	...
	function createClickHandler(item, entry)
	{
		...
		return function ()
		{
			if (descDiv == null)
			{
				...
				descDiv.setAttribute('class', 'ko-calendar-event-info')
				...
				bodyDiv.innerHTML = entry.description != null ? "<i class='material-icons'>event</i>"+" "+entry.description : "";
	...
	function createListEvents(titleId, outputId, maxResults, autoExpand, googleService, calendars, titleFormat)
	{
		...
		function processFinalFeed(feedRoot) {
			...
			if (len == 0) {
				NothingToShowTitle = document.createElement("div");
				NothingToShowTitle.setAttribute('class','ko-calendar-no-events title');
				NothingToShowTitle.appendChild(document.createTextNode("Check back soon"));
				eventDiv.appendChild(NothingToShowTitle);

				NothingToShowBody1 = document.createElement("div");
				NothingToShowBody1.setAttribute('class','ko-calendar-no-events body');
				NothingToShowBody1.appendChild(document.createTextNode("It looks like we don't have any upcoming events."));
				eventDiv.appendChild(NothingToShowBody1);

				NothingToShowBody2 = document.createElement("div");
				NothingToShowBody2.setAttribute('class','ko-calendar-no-events body');
				NothingToShowBody2.appendChild(document.createTextNode("Have an idea for an event? "));

				EmailUs = document.createElement("a");
				EmailUs.setAttribute('href', "mailto:".concat(contactEmail));
				EmailUs.appendChild(document.createTextNode("Email us"));
				NothingToShowBody2.appendChild(EmailUs);

				eventDiv.appendChild(NothingToShowBody2);
			}
			else {
				for (var i = 0; i < len; i++) {
					var entry = entries[i];
					...
					if (dateString != prevDateString) {
						...
						dateDivDay = document.createElement("div");
						dateDivDay.setAttribute('class','ko-calendar-day');
						dateDivDay.appendChild(document.createTextNode(startJSDate.toString("dd")));
						dateDiv.appendChild(dateDivDay);
						dateDivMonth = document.createElement("div");
						dateDivMonth.setAttribute('class','ko-calendar-month');
						dateDivMonth.appendChild(document.createTextNode(startJSDate.toString("MMM")));
						dateDiv.appendChild(dateDivMonth);
					...
					li.appendChild(dateDiv.cloneNode(true));
					li.setAttribute('class','event-body');
					...
					*/
					{
						...
						li.onclick = createClickHandler(li, entry);
	...
	result.loadCalendarDefered = function(apiKey, titleId, outputId, maxResults, autoExpand, calendarUrl, calendarUrl2, calendarUrl3, titleFormat, contactoEmail)
	{
